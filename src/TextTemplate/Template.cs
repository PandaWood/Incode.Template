...
namespace TextTemplate
{
	...
	/// </typeparam>
	/// <example>
	///   Basic template using C# as programming language
	///   <code>
	...
	public class Template<T> : IDisposable
	{
		...
		private string PreProcessCode(string content, HashSet<string> inclusionStack)
		{
			...
			foreach (var reference in parser.AssemblyReferences)
				sb.AppendFormat("<%@ references {0} %>{1}", reference, Environment.NewLine);
			foreach (var import in parser.NamespaceImports)
				sb.AppendFormat("<%@ using {0} %>{1}", import, Environment.NewLine);

			foreach (var part in parser.CodeParts)
				if (part.StartsWith("<%@", StringComparison.Ordinal))
				{
					var subContent = part.Substring(3, part.Length - 5).Trim();
					if (subContent.StartsWith("include ", StringComparison.OrdinalIgnoreCase))
					{
						var name = subContent.Substring(7).Trim();
						if (inclusionStack.Contains(name))
							throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, 
								"A cycle is detected in included content, was triggered by {0}, stack already contains {1}", 
								name, string.Join(", ", inclusionStack.ToArray())));
		...
		{
			var templateDomainFriendlyName = "TextTemplate TemplateDomain #" + ++_TemplateIndex;
